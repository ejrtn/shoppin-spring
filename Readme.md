...
- 주황색 : 장바구니 추가
- 보라색 : 구매 페이지로 이동
- 하단에 상품 리뷰가 나오며, 10개씩 불러올 수 있다
더보기 버튼은 다음 리뷰 데이터가 존재하면 화면에 보여진다(더보기 버튼이 존재 한다는 것을 알리기 위해 강제로 보이게 함)
...
- 관리자 페이지에서 상품목록, 추가, 삭제 및 사용자와 채팅 상담이 가능합니다
- 빨강색 : 사용자가 채팅으로 직접 문의를 할 수 있는 창, 오직 텍스트만 입력 가능
- 노랑색 : 현재 날짜-시간 기준 최근 7일 채팅
- 초록색 : 새로운 채팅
...
- 큰 카테고리, 작은 카테고리 선택
- 이름, 가격, 할인가 입력
- 컬러, 사이즈, 수량 입력을 하며, "+", "-" 클릭으로 추가 또는 삭제 가능("+"는 마지막에만 존재)
