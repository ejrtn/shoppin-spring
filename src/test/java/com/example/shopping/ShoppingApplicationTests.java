...
//		try {
//			productService.productSave(productDto, new MockMultipartFile("피카츄.png","피카츄.png", MediaType.IMAGE_PNG_VALUE,new FileInputStream(System.getProperty("user.dir")+"\\..\\피카츄.png")));
//		}catch (Exception e){
...
//		deliveryService.searchWaybill("2025-03-29 ~ 2025-03-29",deliveryDto.getDeliveryId(),"01");
		...
		img = "2318446_16783280722001_big.webp";
		name = "비즈 보트 슈즈 브라운";
		productDto.setPrice(43900);
		productDto.setCategory("신발");
		productDto.setSmallCategory("기타");
		productDto.setDiscount(71);
		productId = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
		productDto.setProductId(productId);
		productDto.setCnt(10);
		productDto.setExplanation(name);
		productDto.setProductName(name);
		try {
			productService.productSave(productDto, new MockMultipartFile(img,img, "webp",new FileInputStream(System.getProperty("user.dir")+"\\..\\"+img)));
		}catch (Exception e){
			System.out.println(e.getMessage());
		}
